...
- Host reboots / OS major upgrades / dnf-system-upgrade: not yours. Propose; human decides. (Carve-out: the SELinux **setup-completion convergence** below performs the one-time automatic reboot as sanctioned SETUP machinery — that is not a steady-state agent reboot.)
- Host SELinux (v1.2.49+): host target is **enforcing**, reached by a **no-wait** one-time convergence. From a disabled host `setup-host.sh` sets `SELINUX=permissive` + `/.autorelabel`; the Day-0 reboot relabels **in permissive** (brick-safe), auto-reboots, and a **fire-once** `selinux-enforce-once` unit then flips to enforcing **live** (`setenforce 1` + config) on the now-labeled boot and self-disarms — **2 reboots, no soak, no health-check, no auto-revert** (all dropped in v1.2.49: a data-less VPS just re-provisions if enforcing ever wedges). Permissive-first-relabel is the one brick-safety kept (enforcing never runs against an unlabeled fs). Steady state is plain **enforcing**. **The agent NEVER**: sets/flips enforcement live (`setenforce`, hand-editing `/etc/selinux/config`), edits the flip unit/marker on the live host, or disables SELinux — all of that is **propose-and-surface** (the logic lives in `setup-host.sh` + `selinux-enforce-once.sh`, applied only by an operator `setup.sh` re-run). The first reboot stays operator-initiated. Per-host opt-out: `SELINUX_TARGET=permissive ./setup.sh`. The fedora-dev workload container stays SELinux-**exempt** (`label=disable` — nested rootless podman + fuse-overlayfs; host enforcing does not touch it; do NOT "harden" that away). Never disable host SELinux.
