...
Maintainership of a workload image repo is now **SHARED** with the relevant **build dev box** — `fedora-dev` for Fedora images, `debian-dev` for Debian (the per-distro-family build agent that already develops those repos; there is **no** per-image dev box — `fedora-desktop`'s dev box *is* `fedora-dev`) — coordinated by the maintainer's clickable approval on every merge, not by exclusive ownership. **Building images is always CI's job, never `podman build` on this host** — that boundary is unchanged. Repos OUTSIDE the oso-gato fleet (or that I neither operate nor can diagnose) stay off-limits: surface a diff, never a PR.
...
*runtime* path frequently cannot be exercised anywhere else: the build dev box (`fedora-dev` /
`debian-dev`) builds in a nested engine and can't reach a real deployment, while **only this host runs the live containers**
