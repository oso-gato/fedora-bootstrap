...
# Version: 1.2.70 (rebuild-devbox RESUME-PRESENTATION + POLLER-NOT-A-BLOCKER — two fixes the first real R17 rebuild surfaced (2026-07-19). (1) RESTORE-INTO-'main': each session is restored as a WINDOW of the shared `main` tmux group (`new-window -t main:`), not a standalone `new-session -s <name>` — every interactive login joins the `main` group (`destroy-unattached on`) and a grouped client sees ONLY `main`'s windows, so a standalone restore was INVISIBLE to the operator's reconnect and forced a manual re-resume; resume + nudge + active-probe all now target `main:<name>`. (2) POLLER-NOT-A-BLOCKER: the FINISH verdict is `ok==total ⇒ DONE` — a cold poller-service that has not yet swept at check time (a fresh box reassembles the claudebox before poller-service launches) is DONE-WITH-A-NOTE, never a false FAILED; a genuine session loss (ok<total) still FAILS loudly. Sessions are the rebuild's success criterion; poller liveness is independently watched (#173). Covered by 3 new dryrun rows + mutations M5/M6.)
#
