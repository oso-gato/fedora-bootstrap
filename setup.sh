...
# Version: 1.2.66 (rebuild-devbox RESUME-TO-ACTIVE: a rebuilt box's restored sessions now come up ACTIVELY CONTINUING, not merely present. A BOX-READY gate (assembled+enterable, re-checked across ticks, deadline-bounded) stops the launch landing in a still-assembling box and falling back to a bare shell — the 0/N-idle race that resumed 0/2 on the first real rebuild — and each session is then submitted a continue-nudge (literal text + a discrete Enter) asking it to touch a per-sid marker, which the executor polls as a filesystem handshake to CONFIRM it received+submitted+executed. Honest three-way tally: handshake-confirmed=working, claude-up-but-unconfirmed and bare-shell-idle named distinctly. Empirically established (tmux + the real claude TUI): a positional prompt only pre-fills, and a live session's transcript does not flush in-tick, so neither is a usable liveness signal. No operator steps. fedora-dev R17 option-b.)
#
