...
# Version: 1.2.76 (increment 2 — apply-bootstrap RECREATE: a merged change that rewrites a deployed workload Quadlet's env/secret now takes effect on the RUNNING container. host-apply.sh sha256's each ~core/.config/containers/systemd/*.container BEFORE vs AFTER the apply and records the CHANGED workloads to a world-readable signal; host-agent-watch.sh's apply-bootstrap, on a SUCCESSFUL apply, files an APPROVAL-GATED rebuild-devbox recreate for each changed REBUILDABLE dev box — reusing the proven R17 restore lineage UNCHANGED, so the maintainer's one `approved` tap gates the session drop. Config-converge alone rewrote the Quadlet file but the running container kept its old env; this closes that half. host-apply.test.sh 10/10, rebuild-devbox-dryrun.test.sh 30/30. Prior line: 1.2.75 rebuild-devbox restore polish — the FIRST restored session now lands as tmux window 0, not window 1 behind a leftover bash window 0. v1.2.71's restore always did `new-session -d -s main` first (which mints an empty bash window 0) then added each tenant as a new-window, pushing the sessions to windows 1,2. Now restore_session creates `main` WITH the first tenant as its initial window (`new-session -d -s main -n <name>`) when main does not exist yet, and only adds new-windows for subsequent tenants OR when a main already exists — an EARLY login's bash window is never hijacked (the user may be typing in it; the sessions simply land after it). Matches normal single-session behaviour (claude is window 0). Also swaps the dry-run test's 4 mutation vacuous-guards from `cmp` to `sha256sum` — the live-gate image ships no diffutils, so `cmp` printed `command not found` in #219's GREEN gate log. validation/rebuild-devbox-dryrun.test.sh = 27/27 incl. a new window-0 assertion. Prior line: 1.2.71 rebuild-devbox RESUME made real — visible sessions + honest verdict.) G1: the poller-sweeping VERIFY probed `systemctl --user`/`journalctl` but the box has NO systemd (PID 1 is a bash init; the poller is a background loop) — the probe could NEVER succeed, so EVERY rebuild reported poller=down → FAILED regardless of reality; now it asserts the poller's base-visible HEARTBEAT (~/.local/state/pr-poller/poller.log mtime FRESH). G2/G3: restored sessions were standalone `tmux new-session -s <name>` sessions OUTSIDE the `main` group every mosh/ssh login joins, so they were alive + handshake-confirmed yet the user moshed into a bare shell and re-resumed → duplicates; now each tenant is a NAMED WINDOW in `main` (visible to the login, N tenants stay distinct), selected current so the login lands ON it. G4: the verdict fused poller-liveness with session-restore — a perfect 2/2 restore read FAILED because poller=down, routing the maintainer to a DESTRUCTIVE re-rebuild of a healthy box; now MULTI-DIMENSIONAL — DONE on ok==total, poller-down ⇒ DEGRADED (restart the poller, do NOT re-rebuild), FAILED only when sessions did not restore. G5: restore_session is IDEMPOTENT (a re-entered tick leaves a live session alone, never kill+recreate). G6: the box-ready DEADLINE timed from `.rebuild` (stamped at FIRE) charged the whole health-gated rebuild against the assemble budget → premature FAILED; now from `.assembling` (stamped at rebuild-COMPLETE). G8: re-nudge slice floored at 40s so a received nudge confirms before a second is ever sent. All 26 rows of validation/rebuild-devbox-dryrun.test.sh pass, incl. the new DONE+DEGRADED case + 4 mutations.)
#
