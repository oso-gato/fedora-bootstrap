...
# Version: 1.2.73 (fleet parity — add the anti-stall `Stop` hook to policy/managed-settings.json so it is byte-identical to the fedora-dev anchor. anti-stall-stop.sh's Stop hook was added to fedora-dev's managed-settings but never propagated here, so bin/fleet-guard-parity.sh flagged the drift on every fedora-dev PR. Pure guard-payload alignment; the hook fails open ([ -x "$H" ] && exec … else exit 0), a no-op where the script is absent. Sequences after v1.2.72 / #221.)
#
