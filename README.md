...
Version: **1.2.44** — **Fix — the host SELinux enforce-gate still required the now-removed `fail2ban.service`, silently pinning the host PERMISSIVE.** fail2ban was removed fleet-wide in v1.2.41 (key-only door — nothing to brute-force), but `selinux-autoenforce.sh`'s critical-services health gate still listed `fail2ban.service`, so `gate_once()` could never PASS — a fresh host stayed permissive and an already-enforcing host **auto-reverted** to permissive (and `verify.sh` already asserts fail2ban *absent*, so the two directly contradicted). One-line gate fix + the stale log string. **Control-plane (host SELinux convergence) — security-posture fix, flagged.** Prior: v1.2.43 — **Fix — `claude` sometimes fails on the *first* try right after a box rebuild, then works seconds later.** Right after a rebuild the freshly (re)started claudebox's `/dev/pts` is briefly not ready, so the `claude` wrapper's **first** interactive `distrobox enter` fails with `Error: OCI runtime error: crun: ptsname: Inappropriate ioctl for device` — a transient PTY-allocation race, not a broken box (the operator re-runs `claude` and it works). The wrapper now **retries automatically**: `distrobox-enter` ends in `exec "$@"`, so `podman`'s exit code reaches it **verbatim**, and **125/126 are `podman`/OCI exec-setup failures a started session never returns** (claude exits 0/1/130) — so they uniquely mark "couldn't enter yet" and are the **only** codes retried (bounded to **6**, short escalating backoff capped at 4 s, ~18 s total — comfortably past the observed ~15 s warmup window); success, `Ctrl-C` (130), or any genuine non-zero (including the start-phase exit-1 path) is surfaced unchanged. **Distinct from and complementary to v1.2.38**, which fixed concurrent enters *during* a rebuild — this fixes the operator's **own** enter *after* it completes. Box-entry/assemble machinery — **standalone**: adds no privilege and touches no security/merge gate. **Validation:** `bash -n` + a stubbed behavioral harness over the **exact emitted launcher** proves 125/126 retry to success, the loop bounds at 6 retries, and 0/1/130 are never retried (surfaced verbatim) — 8 cases all pass; adversarially verified by 3 parallel review lenses (verdict sound). The PTY race is nondeterministic, so the sole end-to-end proof is the next post-rebuild `claude` on the host. Prior: v1.2.42 — **Security — host SSH door goes all-keys (symmetric with the dev box).** Drops the public-door fingerprint **allowlist**: `sync-authorized-keys.sh` now authorizes **all** keys on `github.com/oso-gato.keys` (the account is the single trust root — the operator manages it directly, so the allowlist added marginal defense-in-depth at the cost of enroll friction + a host/dev-box lockstep). `setup-host.sh` phase 5 drops the per-device `LOGIN_KEY` sshd drop-in (`20-login-key.conf`) and **converges an already-deployed host** (removes the file + reloads sshd, so `PermitUserEnvironment` is no longer set); the `LOGIN_KEY` per-device audit tagging is removed. **Control-plane (key-sync + sshd) — security-posture change, flagged.** Trade-off: any key on the account can use the public door; in exchange, key management is one place (the account). Prior: v1.2.41 — **Security — remove fail2ban from the host (both public ssh doors are key-only).** fail2ban's job is throttling *password* brute-force, but the public ssh doors are **key-only** — no password to guess, so it bought nothing; it ran (journald backend) but guarded nothing. `setup-host.sh` drops `fail2ban-server` from the install + the sshd jail and **converges an already-deployed host** (stop+disable the service, remove the package + any legacy `fail2ban`-metapackage baggage); `verify.sh` now asserts fail2ban **absent**; the PACKAGES row is removed. Paired with the `fedora-dev` drop of `fail2ban` + `rsyslog` (merged). Trade-off: slightly more scanner log-noise on the public doors, in exchange for not running a control with no purpose on a key-only door. **Security-posture change — flagged.** Prior: v1.2.40 — **Docs — correct two false public-exposure claims in the README's Access section.** The table marked the dev container's ssh/mosh as `From public internet: no` and the prose said the public IP "exposes exactly two surfaces" — but this host deploys the `fedora-dev` Quadlet, whose `PublishPort`s expose **public key-only ssh on :4444 + mosh on 61001-62000/udp**, a third hardened public surface. The table cell + the sentence now state this; native RDP/VNC and Cockpit stay tailnet-only. Found during the SSH/Mosh connectivity audit — docs only, no host package/service/deploy-path change. Prior: v1.2.39 — **Docs — the deeper-probe pattern is now in the `.live-gate` schema reference.** `validation/live-gate.sample` documents that a target's `PROBE` may invoke a *shipped script* (COPY'd into the image, no Containerfile change) for richer, lineage-aware assertions — the desktop session actually rendering, the RDP-server config, the auth chain, multi-user per-user ports — all inside the loopback candidate, self-diagnosing; a real RDP-pixel frame stays operator-run (it needs a published port + host-side tools). Proven by fedora-desktop's deeper Gate-B probe (zero host/apparatus change — the gate is workload-agnostic). Docs only — no host behavior change. Prior: v1.2.38 — **Fix — the update mechanism: every auto-update / `claudebox-rebuild` "built a box then failed", while day-zero succeeded.** Root cause is a concurrency race, not a build defect: the v1.2.29 **15 s live-gate watcher** fires `distrobox enter claudebox` *throughout* a rebuild, racing `setup-user.sh`'s own first-enter — the one that runs `distrobox-init` (the `dnf install`). Two `distrobox enter` then drive `distrobox-init` **concurrently** in the same fresh box, where they either **collide on the pre-init hooks** (init errors) or **deadlock on distrobox's name-keyed `~/.cache/distrobox/.claudebox.fifo`** (init hangs); the v1.2.30 **480 s `TimeoutStartSec`** then turns the hang/contention into a hard SIGTERM kill. Day-zero is immune only because the watcher's timer is enabled *after* the box is built (`user 4/5`). **Four-part fix:** (1) `box-rebuild.sh` **stands the watcher down** (`stop live-gate-watch.timer` + `.service`) for the whole rebuild window, with a `trap` that re-arms it even on failure; (2) `live-gate-watch.service` gains an **`ExecCondition`** that cleanly **skips** any firing while `claudebox-rebuild-run.service` is active (defense-in-depth); (3) `setup-user.sh`'s first-enter is now an **authoritative readiness barrier** — it asserts `claude-code` is actually installed rather than trusting `-- true` (which a concurrent enter can make return early); (4) the run-service **`TimeoutStartSec` is raised 480 → 1800 s** (fleet standard), since 480 assumed a race-free rebuild. **Control-plane** (box-rebuild/assemble machinery). **Validation:** the failure was **reproduced** and the fix **validated disposably** on a throwaway box via `systemd-run --user` on the host (faithful: local podman, lingering-manager env) — race-free build GREEN, concurrent-enter build RED (a pre-init-hook error and a hung deadlocked init across runs), watcher-skipped-during-rebuild build GREEN (37 skips); `bash -n` clean. A real `claudebox-rebuild` on the live host is the sole end-to-end proof. Prior: v1.2.37 — **Fix — the v1.2.35 ordering bug: `setup.sh` built the claudebox *before* writing the policy, so a re-run on an already-deployed host still failed at `user 3/5` with "rejected by policy".** `setup-user.sh` wrote/repaired the image-trust `policy.json` in `user 4/5` but assembled the claudebox in `user 3/5` — so on a host whose restrictive policy lacks the toolbox scope, the assemble's `quay.io/fedora/fedora-toolbox` pull is rejected *before* the repair runs. A small idempotent guard now trusts the toolbox base in the live `policy.json` **before** the assemble (no-op on a fresh host / when already present). v1.2.35 fixed fresh day-zero hosts; this fixes the re-run-on-deployed-host path. Host-only. **Validation:** `bash -n` + guard-logic simulation only — **not** run on a live host; a real `setup.sh` re-run is the sole end-to-end proof. Prior: v1.2.36 — **Release process slimmed — the per-release git tag is dropped (it added no value here).** The host deploys `main` (not tags), rollback is `git checkout <commit>` + re-run `setup.sh`, nothing external pins a release, and the version-of-record is already in-tree (`VERSION` + `setup.sh` header + this front-matter + the README "Upgrading" changelog) — so a per-release tag was redundant friction, and it was already abandoned after `v1.2.19` while the version climbed to 1.2.35 with no ill effect. `CLAUDE.md` (RELEASING + RELEASE-DOC) and the agent-law `policy/CLAUDE.md` drop the tag step + the immutable-tags rule, and the stale `git push origin main` release step is corrected to the PR-merge model; tagging stays optional for a deliberate milestone. **Control-plane** (touches `policy/CLAUDE.md`) — docs/process only, no host change. Prior: v1.2.35 — **Fix — the host `claudebox-rebuild` failed after the first build because the image-trust `policy.json` didn't trust the toolbox base.** The host's default-`reject` `policy.json` allowlisted `ghcr.io/oso-gato` + `registry.fedoraproject.org/fedora` + `containers-storage`, but **not** `quay.io/fedora/fedora-toolbox` — the claudebox base pinned in `distrobox.ini`, which distrobox **re-pulls on every `claudebox-rebuild`**. So the rebuild was rejected (`Source image … is rejected by policy`, exit 125), breaking every rebuild **after** the first — the first day-zero assemble runs *before* `setup-user.sh` writes the policy (under the permissive default, caching the toolbox); only later rebuilds re-pull under the restrictive policy. `setup-user.sh` now trusts the toolbox base in **both** the create-heredoc (fresh hosts) and the idempotent merge (repairs an already-deployed host in place on the next `setup.sh`). Host-only — no package/service/deploy-path change; `fedora-dev`/`fedora-desktop` are unaffected (they run no restrictive policy). Prior: v1.2.34 — **Throwaway-churn hardened — the host pre-merge BUILD now serves churn from a persistent dnf cache, and a crash-safe sweeper + bounded cache-GC keep the throwaway mechanism from accumulating orphans or exhausting the VPS quota.** The host top-level engine is not chroot-isolated, so `build-candidate.sh` now binds a **persistent dnf RPM cache** (`$HOME/.cache/fd-dnf` → `/var/cache/libdnf5:rw`): RPMs a candidate downloads persist across **all** later candidate builds, so a forced re-run serves them from disk instead of re-downloading (measured **94 s → 33 s** on a forced in-box re-run). The disposable tag + EXIT-trap `rmi` + no `--no-cache` are unchanged. A new **`throwaway-sweep.sh`** (invoked at `live-gate-watch.sh` start, self-throttled; also wirable to a periodic timer/cron) reaps the orphans a `kill -9`/crash leaves that the per-run EXIT traps miss — **aged** (>`FD_SWEEP_AGE_MIN`, default 120 m) `localhost/disposable/*` images + `vcand-*` containers + throwaway source trees — age-gated, non-forced `rmi`, and `flock`-guarded so a running gate's current artifacts are never touched. The same reaper **bounds the persistent dnf cache** by age then size (RPMs older than `FD_DNF_CACHE_MAX_AGE_DAYS`, default 45 days, pruned first; then LRU size-prune to `FD_DNF_CACHE_CAP_GB`, default 15 GB) and the dangling layer cache via `podman image prune --filter until=FD_BUILDCACHE_AGE`, default 168 h, so churn can't exhaust the disk. All caps are overridable env vars. Host-only validation tooling; no host package/service or production deploy-path change. Prior: v1.2.33 — **Live-gate hardened — a labelled PR's `.live-gate` is now consumed *safely* (parsed, never executed) and fences are loopback-only-enforced.** The live-gate is now **dynamic** (Model C, on this branch): the watcher discovers every `live-validate`-labelled open PR **org-wide** (no per-repo list), each PR's repo is **cloned on demand** (PR head into an ephemeral tree), gated against an **in-repo `.live-gate` contract**, with a **structural guard** that neutrally skips a non-buildable repo. Two security fixes complete it: **(1) [host RCE]** the consumer used to `source` the PR-shipped `.live-gate` as host shell — so a `FENCE_x="$(cmd)"` or `KEY=1; cmd` line ran arbitrary code **on the host**. It now **parses** the contract (reads `KEY=VALUE`, validates the key, strips one quote layer, assigns via `printf -v` — never `eval`/`source`); single-quoted command values are inert here and run only **inside the fenced container**, command substitution outside single quotes is **refused**, and a hostile/malformed contract is **rejected RED, not executed**. **(2) [no port leak]** the gate now **fails closed** if a resolved fence carries a publish flag (`-p`/`--publish`/`-P`/`--publish-all`) — it is loopback-only by construction. No production deploy-path change. Prior: v1.2.32 — **Multi-device tmux docs — the v1.2.31 latest-input-wins geometry is now documented fleet-wide.** The README gains a "Multi-device sessions (tmux geometry)" section and its stale per-`LOGIN_KEY` tmux bullet is corrected to the single shared `main` group; `FLEET.md` "Shared invariants" gains a "Multi-device terminal" bullet; and Build Principle 8 (VALIDATE) now requires proving terminal/TUI rendering with a REAL multi-client PTY harness (a naive byte VT model mis-reads UTF-8 fills like `·` as garbage). Docs only — the runtime `tmux.conf` shipped in v1.2.31. Prior: v1.2.31 — **tmux latest-coview — the shared `main` session now follows whichever device most recently sent input.** `/etc/tmux.conf` defaults to `window-size latest` (seamless macOS↔iPad handoff over mosh: the idle device blank-letterboxes/crops cleanly and reclaims full size on its next keystroke; when the active device disconnects the session falls back to whoever remains), adds `fill-character ' '` (an idle larger device's surplus is blank, not the `·` middle-dot that read as garble), and `prefix+g` to cycle `latest → smallest → largest`. Root cause proven against tmux 3.6 source + a live multi-client harness: a tmux window has exactly ONE size shared by all co-viewing clients, so differently-sized devices on the same tab can't each be full-size — `latest` makes the active device win and the rest degrade cleanly. Supersedes the v1.2.18 `window-size smallest` choice. `setup-host.sh` re-stamp only. Prior: v1.2.30 — **Rebuild backstop tightened — the box-rebuild crash/hang timeout drops 900 s → 480 s.** A practical exam on this VPS (egress 46 MB/s, but a rebuild is dnf-bound: install 101 s, full rebuild ≈ 150 s, worst-case legit — cold 2.17 GB pull + first-enter retries — ≈ 450 s) showed 900 s was ~2× the worst case. `claudebox-rebuild-run.service` `TimeoutStartSec` is now **480 s** — clears the worst-case legit rebuild with margin, halves crash recovery, and never clips a healthy rebuild (those are detected complete in ~2 s; the backstop only fires on a *stuck* rebuild). Host-only — the host serializes the rebuild via systemd, so the fd-9 lock leak that hangs the workload boxes does not apply (the matching workload fix ships in `fedora-dev`/`fedora-desktop`). Prior: v1.2.29 — **Live-gate cadence — the watcher now polls every 15 s (was 5 min).** For snappy turn-around during active development, `live-gate-watch.timer` drops from 5 min to **15 s** (`AccuracySec=1s` is required for systemd to honor a sub-minute interval; the `RandomizedDelaySec` jitter is removed). Safe at 15 s: the watcher self-serializes via `flock`, so while a build is running the intervening firings skip instantly; an idle firing is a cheap box-enter + one `gh pr list` (well under rate limits). The build+gate (~90 s+) dominates the total turn, so this is mostly pickup latency. Prior: v1.2.28 — **Live-gate transport fixes (both proven).** Two robustness fixes to the loop transport: (1) the watcher now runs **inside the claudebox** (`live-gate-watch.service` execs `distrobox enter claudebox`), because `gh`/`git` are box tools not on the minimal host — it polls `gh` + drives the host engine via the box's `CONTAINER_HOST` bridge (build/run still land on the host engine); the previous host-only service failed `gh pr list`. (2) `setup.sh`'s workload-clone update no longer **aborts on a dirty working tree** — `~/<name>` is setup-managed, so it discards stray edits then `git merge --ff-only origin/main`, *keeping* the force-push detection (a blind `reset --hard` would silently accept a force-push to `main`). Prior: v1.2.27 — **Live-gate loop closed — fedora-dev labels a PR, the host builds + gates it, and posts the verdict back.** `live-gate-run.sh` gates one PR (build the candidate disposably → `validate-candidate.sh` Gate B → `gh pr comment` the GREEN/RED verdict; the host **comments, never merges**). `live-gate-watch.sh` (a `systemd --user` timer) polls `live-validate`-labelled PRs, dedups per-commit, and invokes the runner — running a **disposable** container that never touches the live workload, so it is **not gated on the dev session** (validation runs *while* fedora-dev is in active use — the loop's point). **Proven end-to-end: a labelled fedora-dev test PR was built-from-source + gated GREEN and the verdict posted back to the PR; the watcher discovered + deduped it.** Closes the loop — fedora-dev opens/labels a PR → host builds + gates + verdicts → fedora-dev iterates (RED) or you merge (GREEN), human at exactly one point. Prior: v1.2.26 — **Live-gate builder — the host can now build a PR candidate disposably and gate it (the loop's linchpin).** `build-candidate.sh` exports a PR ref to a **throwaway source tree** (`git archive` → temp dir, no clone mutation), `podman build`s a **disposable** candidate (`localhost/disposable/<name>:val-<sha>`; default isolation works on the host's native-overlay top-level engine — no `--isolation=chroot`), hands it to `validate-candidate.sh` (Gate B), then `rmi`s it (base layers stay cached for the next churn). **Proven end-to-end: the host built fedora-dev from source and gated it GREEN** — the pre-merge build→gate path CI (no PR publish) and the dev box (can't live-run PID-1) cannot do. `setup-user.sh` now installs `build-candidate.sh` + `validate-candidate.sh` (the latter was an uninstalled orphan). Sanctioned by the v1.2.25 carve-out — never pushed, never deployed. Prior: v1.2.25 — **Policy carve-out — the host may now build _disposable validation_ images.** The host law (`policy/CLAUDE.md` + project `CLAUDE.md`) is split into two classes: building a **shipping** image (published/pushed/deployed) stays **always CI's job, never on the host**; building a **throwaway validation** image is now **permitted** — *solely* to live-test an open PR **before merge** (CI publishes nothing pullable on an open PR, and the dev box's nested engine can't live-run a PID-1 image, so the host is the only surface that gives a faithful pre-merge verdict). Permitted only when `localhost/disposable/*`-tagged (never a `ghcr.io/oso-gato` deploy ref), **never** `podman push`ed (the host holds no `write:packages` credential), run `--rm` + `rmi`'d, never a `WORKLOAD_CONTAINERS` member — exactly what `validation/rollback-spike.sh` already does. **The deploy path + shipping-image provenance are unchanged** (CI-built + cosign-signed). Prior: v1.2.24 — **Default effort — the host claudebox now starts every session at ultracode.** The `claude` wrapper injects `--settings '{"ultracode":true}'` so each session begins in **ultracode** (`xhigh` effort + workflow-by-default), and `policy/managed-settings.json` gains a top-level `"effortLevel": "xhigh"` as the persistent floor for any path that doesn't go through the wrapper (subagents, a direct `/usr/bin/claude`). Ultracode is **session-scoped and ignored in settings files**, so the wrapper is the only place it can be made a default. With v1.2.20's `defaultMode: auto`, a fresh `setup.sh` re-run now brings the box up **in auto mode at ultracode with no per-session action** — completing the autonomous-defaults set. The **merge gate is unchanged** — the `gate-push.sh` `ask` hook, the `git push` / `gh pr merge` deny rules, and `disableBypassPermissionsMode` all remain. Prior: v1.2.23 — **Loop gate B proven — the pre-merge live candidate gate is faithful.** `validate-candidate.sh` is parameterized: `CAND_FENCE` supplies the candidate's run-contract (caps/devices, minus public ports + real secrets; default = hardest untrusted fence) and `CAND_PROBE` is the "does it actually serve" assertion, run on the candidate's OWN loopback via `podman exec`. Demonstrated faithful — a correctly-serving candidate **PASSES**, while one that is *healthy but serves HTTP 500* is caught by the probe and **FAILS** (the access-path probe is load-bearing, not just health). Validation tooling; no production behavior change. Prior: v1.2.22 — **Loop gate A proven — the deploy-resilience rollback backstop fired for the first time.** `validation/rollback-spike.sh` now goes **GREEN**: a never-healthy `:latest` is retagged to the prior digest, restarted, and recovers with a `.rolled-back` marker. Two reusable fixes it surfaced — the spike's throwaway Quadlet now uses an explicit `HealthCmd=` (OCI drops a built image's `HEALTHCHECK`), and `container-refresh.sh` gains a **`BUSY_PROBE` seam**: default unchanged (the claudebox probe), overridable to an empty probe for non-claudebox workloads (the documented mechanism, previously missing). No production behavior change. Prior: v1.2.21 — **Fix — the image-trust `policy.json` writer produced an invalid `containers-storage` entry that broke every pull.** `setup-user.sh` wrote `"containers-storage": [ … ]` (a bare requirements array) where containers-image requires a scope→requirements **object** `{ "": [ … ] }`; podman rejected the entire policy (`JSON object expected, got 91`), so no `ghcr.io/oso-gato` image could be pulled and `fedora-dev` was stuck restart-looping. Both writers are fixed, the idempotent merge now **repairs an already-broken host in place**, and a fail-closed structural check rejects any non-object transport. Prior: v1.2.20 — **Auto mode — the host claudebox now defaults to the `auto` permission mode.** `policy/managed-settings.json` drops `"disableAutoMode": "disable"` and adds `"defaultMode": "auto"`, so in-box Claude sessions start in auto mode — no routine permission prompts, with a background classifier vetting each action before it runs. The **merge gate is unchanged**: the managed `gate-push.sh` `ask` hook, the `git push` / `gh pr merge` deny rules, and `disableBypassPermissionsMode` all remain, so nothing reaches `main` without your explicit approval. Re-stamped into `/etc/claude-code/managed-settings.json` on every `setup.sh` run. Prior: v1.2.19 — **Policy — permit the class-(a) Fedora base + local copy for host-validation fixtures.** The host image-trust `policy.json` (default `reject`, previously trusting only `ghcr.io/oso-gato/*`) gains two scoped entries — the class-(a) Fedora base `registry.fedoraproject.org/fedora` (so the `validation/` host-validation spikes+gates can pull stock Fedora test fixtures) and the `containers-storage` transport (local `save`/`load` for the throwaway tar cache those fixtures live in). The **production run-set is unchanged** — workload Quadlets reference only `ghcr.io/oso-gato/*`, `default` stays `reject`, and the `docker` `""` fallback stays `reject`; this widens only what may be *pulled* for disposable host-validation, not what *runs*. `setup-user.sh` writes it fresh on a new host and **idempotently/additively** merges the two entries into an existing `policy.json` without clobbering operator edits (same `insecureAcceptAnything` posture as the `oso-gato` stanza; both upgradeable to `sigstoreSigned` in lockstep later). Prior: v1.2.18 — **tmux single-group — fix the multi-client geometry-race garble.** ssh/mosh/tmux output garbled (input + output) on every client except a freshly-relaunched native macOS terminal. Root cause: every login funnelled into ONE shared session and, with no `tmux.conf`, `window-size=latest` resized that shared window to whichever client was active last, painting the others onto a foreign grid. Now each login gets its **own session inside one shared `main` tmux GROUP** — every client **shares the windows** but keeps **independent geometry/redraw** (no race) — plus a new `/etc/tmux.conf` (`default-terminal tmux-256color`, `window-size smallest`, `aggressive-resize on`, `client-attached`/`-resized` → `refresh-client`). A single `main` group (not per-`LOGIN_KEY`) is deliberate: the primary path is **keyless Tailscale SSH**, which never sets `LOGIN_KEY`, so a per-key model would collapse to `main` on the tailnet anyway *and* fragment the workspace across access methods — one group = one continuous workspace over tailnet ssh, public ssh, and mosh (`LOGIN_KEY` retained for audit only). Identical model to `fedora-dev`'s fix. Prior: v1.2.17 — Docs: add `FLEET.md` (the swarm map) + a "Where this sits — the fleet" table to the README (no host behavior change; identical `FLEET.md` across all three repos — the human-readable mirror of the `policy/CLAUDE.md` `THE FLEET` block). Prior: v1.2.16 — a Day-0 wizard (`day0.sh`) that ASKS for the Tailscale auth key**, mirroring the workload `spin-up.sh`. Run it as the last Day-0 line: it reads the key from the terminal (**Enter** = browser web-login), runs `setup.sh`, then prompts for core's password and reboots into the SELinux convergence (`SELINUX_TARGET=permissive` ⇒ no reboot). **`setup.sh` is unchanged** — it still honors env `TS_AUTHKEY` and never reboots, so the fully-scripted `TS_AUTHKEY=… setup.sh < /dev/null` + `passwd core && reboot` path still works. Prior: v1.2.15 — spin-up path made explicit; v1.2.14 — day-0 TS_AUTHKEY prompt; v1.2.13 — docs cleanup (scrub deleted-repo refs); v1.2.12 — FLEET governance (3-box model, host PR-only, `fedora-dev` sole merge box); v1.2.11 — BUILT promotion gate (managed `PreToolUse` hook + hardened `managed-settings.json` + CI diff-guard); v1.2.10 — HEADLESS binding prerequisite fleet-wide; v1.2.9 — Principle 3 (MINIMAL) refined fleet-wide (*"minimum" is relative to the chosen capability* + disclosed irreducible hard-dep closure; a lighter option that *reduces* function — e.g. noVNC vs Guacamole's RDP-grade web gate — is a recorded **capability trade-off, not a minimalism win**); v1.2.8 — Principle 2(c) bounded official-upstream-binary class; v1.2.7 — PR-first + maintainer-approved-merge maintainership; v1.2.5 — `verify.sh` fail2ban euid-gate fix; v1.2.4 — genesis/mother-platform role + `fedora-dev` maintainership.
