...
One script that turns a fresh cloud server into your **"mother platform"** — a locked-down host that runs your whole fleet of container apps. Its on-board Claude (the "host claudebox") has **two standing jobs**: it **operates and maintains the host itself**, and it **develops and maintains the source of the fleet's container images** — with the **same development authority as the `fedora-dev` build box** (equalised v1.2.12):

1. **The foundation** — `fedora-bootstrap` (this one, the host's own machinery) + `fedora-dev` (the first workload + the template later ones follow).
2. **Any workload image it runs** — `fedora-desktop` and every other app in the fleet — because the host is the only place that watches them run live, so it's the right place to diagnose and patch them.
...
- 🔧 **Why it can develop those repos:** it's the **first container that ever comes up — the foundation everything else stands on** — *and* the only vantage that can probe the live containers to see how and why an image fails. So it's trusted to **open a PR and, on your explicit click, merge it itself**: repairing the foundation *in place* (like fixing a running car) and turning first-hand diagnosis of any app it runs into a patch. A repo it does **not** run, it only *proposes* changes to. (It still **never builds** images — CI does.)
...
Version: **1.2.12** — Policy: **host-claudebox dev authority equalised to the `fedora-dev` build box.** The genesis claudebox may now **DEVELOP + PR + clickable-approved-merge ANY fleet image source repo it operates and can diagnose live** (`fedora-desktop` + every `WORKLOAD_CONTAINERS` image) — not just the foundation `fedora-bootstrap`/`fedora-dev`. Rationale: the host is the **only** vantage that can probe/log the live containers, so it should turn that first-hand diagnosis directly into patches (*diagnose-live → patch → PR → approved-merge → CI → workload-refresh pull → redeploy*). Two boundaries **preserved**: image builds stay CI's job (never `podman build` here), and a repo the host neither operates nor can diagnose stays **surface-only**. Also **unifies the merge-rule wording** fleet-wide — ONE canonical statement (*propose → clickable approval → the **agent** merges, never a human*), and the `gate-push.sh` deny message now states that procedure at the moment of action. **Policy re-stamp only — no host-layer change**; re-run `setup.sh` to re-stamp the agent law. Server-side **branch protection on `main`** remains the PRIMARY backstop, the operator's one-time item. Prior: v1.2.11 — BUILT promotion gate (managed `PreToolUse` hook + hardened `managed-settings.json` + CI diff-guard); v1.2.10 — HEADLESS binding prerequisite fleet-wide; v1.2.9 — Principle 3 (MINIMAL) refined fleet-wide (*"minimum" is relative to the chosen capability* + disclosed irreducible hard-dep closure; a lighter option that *reduces* function — e.g. noVNC vs Guacamole's RDP-grade web gate — is a recorded **capability trade-off, not a minimalism win**); v1.2.8 — Principle 2(c) bounded official-upstream-binary class; v1.2.7 — PR-first + maintainer-approved-merge maintainership; v1.2.5 — `verify.sh` fail2ban euid-gate fix; v1.2.4 — genesis/mother-platform role + `fedora-dev` maintainership.
...
2. **Develop + maintain the fleet's image source** — with development authority **equal to the `fedora-dev` build box** (equalised v1.2.12): it maintains not only **`fedora-bootstrap`** (the host's own machinery) and **`fedora-dev`** (the first workload image, and the template every later workload follows) but **any fleet image it operates and can diagnose live** (`fedora-desktop`, …) — by **PR + your clickable-approved merge** (never a direct push). Builds stay CI's job.

A repo the host **neither operates nor can diagnose** stays **surface-only** — it proposes a diff, and that repo's own build dev box (`fedora-dev` for Fedora, `debian-dev` for Debian) or the operator opens the PR. For **all** images — including those the host claudebox maintains — the build/deploy handoff is one-way and unchanged:
...
Agent-policy + docs only — **no host behavior change**. The host claudebox's **development authority is equalised to the `fedora-dev` build box**: it may now **develop + PR + clickable-approved-merge any fleet image source repo it operates and can diagnose live** (`fedora-desktop` + every `WORKLOAD_CONTAINERS` image), not just the foundation `fedora-bootstrap`/`fedora-dev`. Rationale: the host is the **only** vantage that can probe/log the live containers, so it should turn that first-hand diagnosis directly into patches (*diagnose-live → patch → PR → approved-merge → CI → workload-refresh pull → redeploy*). **Two boundaries preserved:** image builds stay CI's job (never `podman build` here), and a repo the host neither operates nor can diagnose stays surface-only. The release also **unifies the merge-rule wording** — one canonical statement (*propose → clickable approval → the **agent** merges, never a human*) — and surfaces that procedure in the `gate-push.sh` deny message. Host-apply is unchanged: re-running `setup.sh` re-stamps the updated agent law (`policy/CLAUDE.md` + `policy/hooks/gate-push.sh` → the box); no host delta.
...
3. **Build/deploy separation (genesis agent).** Images are always BUILT in CI (never `podman build` on the host) and DEPLOYED by the host claudebox via the pull. The host claudebox is the genesis agent: it operates the host AND — with development authority equal to the `fedora-dev` build box (v1.2.12) — maintains the *source* of `fedora-bootstrap`, `fedora-dev`, **and any fleet image it operates and can diagnose live** (`fedora-desktop`, …), by **PR + the maintainer's clickable-approved merge**. A repo it neither operates nor can diagnose stays surface-only (its own build dev box — `fedora-dev`/`debian-dev` — or the operator opens the PR). The invariant the `policy/CLAUDE.md` rules enforce is **build-in-CI + maintainership scope** (which repos the agent may merge) + **the merge gate** (propose → clickable approval → the agent merges), not a strict develop-vs-deploy agent split.
