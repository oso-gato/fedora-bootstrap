...
Documentation only — **no host package, unit, deploy-path, or security change**. Captures the development-loop mechanic + the ticket system in `FLEET.md` and `CLAUDE.md` (roles + ownership, the `live-validate` pre-merge host live-gate, the PR-as-ticket + label model, the clickable-APPROVE merge spine). Re-running `setup.sh` re-stamps the policy/docs; nothing operational changes.

```sh
cd /opt/fedora-bootstrap
git pull --ff-only origin main
./setup.sh < /dev/null        # docs only; no unit/package/deploy change
```

**Rollback** — docs only; `git checkout` the prior commit.
