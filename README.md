...
Here's how that caching actually holds up across many attempts (it's been measured, not just hoped for). Throwing away a test copy deletes **only that copy and its temporary files** — it never touches the caches. The caches aren't tied to any one attempt; they're **shared by all of them**. There are two: the first remembers each finished build step, so a change that only touches the late stuff (a script, a config file) reuses everything heavy untouched and **downloads nothing at all**; the second is a kept-aside pile of the actual software packages, so even when an attempt *does* change which packages get installed, they're **pulled from that local pile instead of re-downloaded** (measured about three times faster — 94 seconds down to 33 — with only a genuinely new package fetched the one time). Each attempt also builds in its own private workspace under its own one-off name, so two attempts can't trip over each other, and the caches are keyed by content, so they can never hand back the wrong version. And because this runs on a small server with limited disk, three guards keep it tidy: each throwaway deletes itself when it finishes (pass, fail, or crash), a sweeper cleans up anything a hard crash leaves behind, and a size/age cap keeps the caches from ever filling the disk.
