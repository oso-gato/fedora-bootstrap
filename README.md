...
- **Bounded cache GC.** The same reaper caps both persistent caches so churn can't exhaust the VPS quota: the dnf cache to `FD_DNF_CACHE_CAP_GB` (default 5 GB — keeps newest RPMs, prunes oldest over the cap) and the dangling podman build cache via `podman image prune --filter until=$FD_BUILDCACHE_AGE` (default 168 h; **dangling-only**, so tagged `ghcr.io/oso-gato/*` images are never touched). All knobs are overridable env vars.
