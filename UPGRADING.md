...
| v1.2.57 | **HOST AGENT** (`host-agent-watch.sh` + `.service`/`.timer`, 10s) — the host half of the autonomous apparatus (fedora-dev#131 R5). Consumes `host-task` GitHub-issue tickets org-wide (`host-op: <verb>` on line 1), performs the requested **allowlisted** host op (v1 verb: `redeploy <workload>` → `workload-refresh@<workload>`), posts a `host-agent: DONE/FAILED` comment + closes the ticket. This is how the walled-off dev box gets host operations done. **No arbitrary-command verb by design**; a workload arg must be in the known-workloads allowlist. Standing-down mirrors the live-gate watcher during a box rebuild. The standard flow enables it on this host | — |
