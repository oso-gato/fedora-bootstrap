...
| v1.2.69 | **R16 OPERATING SCOPE — host end** (issue #132; fedora-dev#167 R16 rule 5): the host live-gate discovered `live-validate` PRs **org-wide**, so it would build a candidate for ANY labelled repo in the org — the #165 scope leak from the host end. It now honours the SAME maintainer-confirmed scope set as the dev side: a new host **`repo-scope.sh`** (a lockstep mirror of fedora-dev's canonical `bin/repo-scope.sh`, ceiling-only) reads a mirrored **`policy/scope.conf`**, and **`live-gate-watch.sh`** gates every discovered PR on it before any per-PR API call, build or verdict — an out-of-scope PR is skipped with one loud line, **no build, no verdict**. **Fail-closed** to the two apparatus repos (fedora-dev + fedora-bootstrap) when the reader is missing or the config unreadable; a readable-but-empty scope denies everything. Reader + conf ride the **F16 managed manifest** (installed to `~/.config/live-gate/scope.conf`, kept byte-current by the absorber's read-back), and `verify.sh` asserts the gate is functional (a non-scoping gate is a loud FAIL). Mutation-proven by `validation/repo-scope.test.sh` (neuter the gate → the out-of-scope repo reaches the runner). | — |
