...
| v1.2.71 | **Poller arming survives a container recreate (#198).** The dev-box poller's autonomous-merge arming (`POLLER_ENABLED`/`POLLER_ARMED` + the fitness- and host-live-gate merge-anchor identities `auto-merge.sh` checks) lived only in the container's *runtime env* — a live `podman exec` injection — so an R17 rebuild / monthly `workload-refresh` **dropped** it and the poller **and its deadman** (both gated on `POLLER_ENABLED` in the box entrypoint) came up **dead and silent** (2026-07-19: ~1.5 h dark, no merges). `setup-user.sh` now persists it into the INSTALLED `fedora-dev.container` Quadlet (the `(b2)` App-credential idiom — activating a deploy-spec line, not re-injecting), so a recreate preserves it. Scoped to `fedora-dev` only, and only once its own standing App is provisioned (no dev-box identity ⇒ can't merge ⇒ fail-closed, no arming); the host live-gate login is derived from the host-name convention `oso-gato-<host>-claudebox` (the same App this host provisions as `gh_app_host_key`; a wrong login fails closed, never a bad merge). Applied by re-running `setup.sh`; takes effect on the next container recreate. Follows v1.2.70. | — |
