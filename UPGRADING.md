...
| v1.2.70 | **R17 rebuild-devbox: poller-readiness verdict DEFERS across ticks (no false-FAILED on a cold rebuild)** — the FIRST live one-tap rebuild (2026-07-19, ticket #198) restored both sessions **2/2 actively-continuing (handshake-confirmed)** but verdicted a **false `PARTIAL — poller=down`**: the executor checked "is the poller observably sweeping?" in a single **120s in-tick block**, and a COLD `poller-service` must wait `.assembled` + the credential + a self-refresh reload before its first sweep — which outlasts one tick (the new box's first sweep landed ~seconds after the check closed). The poller check now mirrors the `box_ready` **across-ticks** discipline: `poller_sweeping_now` is a single-pass probe and `poller_finish` DEFERS (writes no `.acted`, ticket stays open) up to `DEVBOX_POLLER_READY_WINDOW` (600s), then a `.restored` marker makes the deferred re-entry **skip restore** so the just-resumed sessions are never re-killed; only a poller still down **past** the deadline is a terminal FAILED. Sessions all-idle / up-unconfirmed still fail immediately (not deferred). Mutation-proven in `validation/rebuild-devbox-dryrun.test.sh` (defer within window · deferred re-tick → DONE with restore proven not re-run · past-deadline → FAILED). | — |
