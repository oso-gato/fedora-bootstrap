...
| v1.2.65 | **rebuild-devbox RESUME-BY-ID** (fedora-dev D4/#191): the R17 session manifest gains an optional 4th field — a claude-code session-id (a valid UUID). When present, `host-agent-watch.sh` resumes THAT session with `claude --resume <sid>` instead of the cwd-scoped `claude --continue`, so a multi-tenant rebuild no longer collapses N sessions sharing one cwd (`/home/core`) to the most-recent one — every tenant returns by id. Backward-compatible: a 3-field v1 manifest still resumes with `--continue`. `parse_manifest` now accepts `session <name> <cwd> [<sid>]`; strict-UUID validation on the 4th field also re-closes the space-in-cwd hole (a fumbled cwd-with-a-space splits to a non-UUID 4th field ⇒ rejected). Covered by `--selftest` (parse rows) + `validation/rebuild-devbox-dryrun.test.sh` (behavioral: v1→`--continue`, v2→`--resume <sid>`). | — |
