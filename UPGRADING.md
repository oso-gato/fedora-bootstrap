...
| v1.2.70 | **rebuild-devbox RESUME-PRESENTATION + POLLER-NOT-A-BLOCKER** — the two defects the FIRST real R17 rebuild surfaced (2026-07-19, ticket #198): the executor reported `PARTIAL — poller=down` and the operator had to MANUALLY re-create the tmux windows + re-type `claude --resume <sid>`. **(1) RESTORE-INTO-`main`** — `host-agent-watch.sh` restored each session as a STANDALONE `tmux new-session -s <name>`, but every interactive login joins the shared **`main` GROUP** (`tmux new-session -t main -s c$$`, `destroy-unattached on`) and a grouped client sees ONLY `main`'s WINDOW set — so a standalone restore is **INVISIBLE** to the operator's reconnect (the manual re-resume). Restore now ensures the `main` base then adds the session as a **WINDOW** of it (`new-window -t main:`); the resume, the continue-nudge, and the active-probe all target `main:<name>`, so a reconnecting login lands on the resumed window. Proven on a throwaway (a window added to `main` IS seen by a grouped login; a standalone session is NOT). **(2) POLLER-NOT-A-BLOCKER** — the FINISH verdict required `ok==total AND poller==sweeping` for a DONE, so a proven **2/2 handshake-confirmed** resume was reported FAILED because a COLD `poller-service` had not swept at check time (a fresh box reassembles the claudebox for minutes before poller-service can even launch, so the poller warms up AFTER the sessions). Sessions are the rebuild's success criterion: the gate is now **`ok==total ⇒ DONE`** — a not-yet-sweeping poller is DONE-WITH-A-NOTE (surfaced honestly; a genuinely-dead poller is caught independently by the #173 poller-liveness watchdog + the apparatus-deadman), never a false FAILED; a real session loss (`ok<total`) still FAILS loudly and unchanged. Covered by `validation/rebuild-devbox-dryrun.test.sh` (restore-into-`main` window targeting, cold-poller-DONE-with-note, session-loss-still-FAILs + mutations M5 reverting the standalone restore, M6 re-requiring the poller). | — |
