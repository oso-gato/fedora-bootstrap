...
| host-agent-watch.sh | **the HOST AUTONOMOUS AGENT** (apparatus fedora-dev#131 R5) — the symmetric half of the dev-side poller. Consumes dev→host **`host-task` GitHub-issue tickets** org-wide (one `gh search issues` per tick; flock singleton; per-(repo,issue) `.done` dedup — mirrors `live-gate-watch.sh`), parses the LINE-1 instruction `host-op: <verb> [args]`, dispatches to a fixed **verb ALLOWLIST** (v1: `redeploy <workload>` → `workload-refresh@<workload>`; **no arbitrary-command verb by design**; workload arg must be in the known set), posts a `host-agent: DONE/FAILED` comment + labels + closes the ticket. Runs INSIDE claudebox (gh + CONTAINER_HOST bridge); `--selftest` exercises the pure parser + allowlist. This is how the walled-off dev box gets host operations (redeploy, and future recreate/re-stamp) done autonomously. Driven by `host-agent-watch.timer` (10s), stood down during a box rebuild |
