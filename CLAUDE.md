...
| live-gate-watch.sh | live-gate DISCOVERY (Gate B). One org-wide `gh` query for every `live-validate`-labelled open PR (no repo list), resolves each head SHA, per-`(repo,SHA)` `.done` dedup + `flock` self-serialize, invokes `live-gate-run.sh`. Runs inside claudebox (needs `gh`/`git`; drives the host engine via `CONTAINER_HOST`); a `systemd --user` timer. `LIVE_GATE_WORKLOADS` is an optional allowlist FILTER (unset = org-wide, still scope-gated). **R16 OPERATING SCOPE (issue #132)**: because discovery is org-wide, each candidate repo is checked (via `scope_ok` → `repo-scope.sh check` against `policy/scope.conf`) BEFORE any per-PR API call, build or verdict — an out-of-scope labelled PR (the #165 leak from the host end) is skipped with one loud line, no build, no verdict. Fail-closed to the two apparatus repos (fedora-dev + fedora-bootstrap) when the reader is missing or the config unreadable. **R9 FLEET HALT (fedora-dev#135)**: at the TOP of every tick — after the flock, BEFORE the sweep/discovery/build/post — it reads the maintainer-bound `halt` signal via `fleet-halt.sh`; HALTED / persistently-unreadable ⇒ OBSERVE-ONLY (log, then exit — sweep/build/post NOTHING), un-halt resumes next tick, an in-flight build (a prior tick still holding the flock) is never touched. Fail-closed: a missing/unreadable reader parks the tick |
...
| repo-scope.sh | the HOST-side **R16 OPERATING SCOPE** reader (issue #132) — answers "may the apparatus act on this repo?" against the maintainer-confirmed set in `policy/scope.conf`, so the org-wide live-gate honours the SAME scope as the dev side (the #165 leak from the host end). LOCKSTEP MIRROR of the CANONICAL fedora-dev `bin/repo-scope.sh` (the `fleet-halt.sh`/`gh-app-provision.sh` precedent); the CEILING-ONLY subset — `check`/`list`/`--selftest`, pure decision helpers mirrored VERBATIM — deliberately omitting the dev-only per-session (R27/R28) layer + fitness-gate helpers (the host neither merges nor reviews). `check <repo>` → rc 0 IN / 3 OUT / 4 out-under-unreadable-fallback / 2 usage. FAIL-CLOSED: an unreadable `scope.conf` allows ONLY the two apparatus repos ($SCOPE_OWN — fedora-dev, fedora-bootstrap); a readable-but-EMPTY scope denies everything. A local file read, zero API calls. Runs inside claudebox (pure bash). Covered by `validation/repo-scope.test.sh` |
| validation/repo-scope.test.sh | R16 host-scope regression (issue #132): (1) unit-checks `repo-scope.sh` decisions (in-scope rc 0, out-of-scope rc 3, unreadable→apparatus-only fallback, empty→deny-all); (2) drives the REAL `live-gate-watch.sh` with a stub `gh` returning one in-scope + one out-of-scope labelled PR and asserts only the in-scope one reaches the runner while the out-of-scope one is skipped with the loud R16 line — before any per-PR SHA resolve or build. **MUTATION IN-SUITE**: neuter the `R16-SCOPE-GATE` guard and the out-of-scope repo THEN reaches the runner (the org-wide #165 leak restored) — proving the gate is what stops it. `bash validation/repo-scope.test.sh` → exit 0. No GitHub/network/podman. Run after touching the scope gate |
...
| policy/scope.conf | the **R16 OPERATING SCOPE** set the host live-gate honours (issue #132) — one bare repo name per line; a LOCKSTEP MIRROR of the CANONICAL fedora-dev `policy/scope.conf` (keep in sync both directions). Control-plane SECURITY config: it rides the F16 managed manifest, installed to `~/.config/live-gate/scope.conf` and kept byte-current by the absorber's fail-closed read-back. Read by `repo-scope.sh`. Adding a repo is maintainer-gated (R16 rule 3, name-bound `CONFIRMED`); removing needs no ceremony |
| verify.sh | PASS/FAIL acceptance: sockets, box, claude, policy, host-engine reach, tailnet, box-rebuild units, workload-refresh timers, dnf-automatic timer, fail2ban absent (key-only door), firewalld absent (leaf footprint), sudo doctrine boundary, SELinux config not disabled (permissive or enforcing), **host self-refresh armed** (F16 absorber timer enabled + control clone core-writable + last heartbeat not BLOCKED/FAILED — a non-self-refreshing host is a LOUD FAIL, not silence) |
