...
| live-gate-watch.sh | live-gate DISCOVERY (Gate B). One org-wide `gh` query for every `live-validate`-labelled open PR (no repo list), resolves each head SHA, per-`(repo,SHA)` `.done` dedup + `flock` self-serialize, invokes `live-gate-run.sh` — **passing the resolved sha as arg 3** so the runner gates exactly the sha this dedup marker will name (see the live-gate-run.sh row's HEAD-SHA COHERENCE). Runs inside claudebox (needs `gh`/`git`; drives the host engine via `CONTAINER_HOST`); a `systemd --user` timer. `LIVE_GATE_WORKLOADS` is an optional allowlist FILTER (unset = org-wide). **NO SOFT STOP IS READ AT THE TOP OF THE TICK** — the maintainer-thrown FLEET HALT label was RETIRED 2026-07-30 (R9, `oso-gato/fedora-dev#337`; host half v1.2.80): the reader, its call site here and its test are DELETED, so every tick sweeps unconditionally. Stopping this watcher is now App-key revocation or stopping the container — two stronger stops that need no code and do not depend on GitHub being readable |
