...
| throwaway-sweep.sh | throwaway-churn reaper + cache GC: removes AGED orphan `localhost/disposable/*` images, `vcand-*` containers, and throwaway source trees a `kill -9`/crash left (the per-run EXIT traps miss), and BOUNDS the persistent dnf RPM cache by AGE then SIZE (`FD_DNF_CACHE_MAX_AGE_DAYS` default 45 days pruned first, then LRU size-prune to `FD_DNF_CACHE_CAP_GB` default 15 GB) plus the dangling podman layer cache via `image prune --filter until=`. Age-gated + non-forced + flock-guarded + self-throttled so it never touches a running gate's artifacts; invoked at `live-gate-watch.sh` start, also wirable to a periodic timer/cron |
