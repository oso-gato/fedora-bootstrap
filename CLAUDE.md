...
| setup-host.sh | **system layer**, as root — packages, /etc, system services, tailnet, host dnf-automatic, creates `core` + its rootless prerequisites. **Chowns the control clone `core`-writable** (the one root bootstrap step the F16 self-refresh absorber needs — a `--user` process can't `git merge` a root-owned clone; + a `safe.directory` entry). Idempotent, fail-loud-not-fatal |
| setup-user.sh | **rootless layer**, as `core` — user podman socket, ssh keys, claudebox, Claude policy, the `claude` + `claudebox-rebuild` wrappers + the box-rebuild units, workload-refresh harness, verify (no host privilege). Installs (`hcr_install_from`) + enables + **PRIMES** the F16 absorber once so the host is current the moment setup finishes and verify has a fresh heartbeat |
...
| container-refresh.sh | per-workload refresh: busy-probe + pull + digest compare + `systemctl --user restart <name>.service` + rollback on health failure. **`FORCE_REBUILD` seam (R17)**: a purposeful kill+recreate reuses this SAME health-gate + rollback but skips the busy-defer (a rebuild deliberately kills an active session) and the digest-unchanged short-circuit (a lifecycle rebuild recreates even on the same image); UNSET everywhere else (zero behaviour change), set only by `workload-rebuild@` |
...
| live-gate-watch.sh | live-gate DISCOVERY (Gate B). One org-wide `gh` query for every `live-validate`-labelled open PR (no repo list), resolves each head SHA, per-`(repo,SHA)` `.done` dedup + `flock` self-serialize, invokes `live-gate-run.sh` — **passing the resolved sha as arg 3** so the runner gates exactly the sha this dedup marker will name (see the live-gate-run.sh row's HEAD-SHA COHERENCE). Runs inside claudebox (needs `gh`/`git`; drives the host engine via `CONTAINER_HOST`); a `systemd --user` timer. `LIVE_GATE_WORKLOADS` is an optional allowlist FILTER (unset = org-wide). **R9 FLEET HALT (fedora-dev#135)**: at the TOP of every tick — after the flock, BEFORE the sweep/discovery/build/post — it reads the maintainer-bound `halt` signal via `fleet-halt.sh`; HALTED ⇒ OBSERVE-ONLY (log, then exit — sweep/build/post NOTHING), un-halt resumes next tick, an in-flight build (a prior tick still holding the flock) is never touched. An UNREADABLE signal is NOT a halt (fail direction inverted 2026-07-30): the reader returns `CLEAR`/rc 0 and the tick PROCEEDS — a GitHub blip no longer takes the outage-repair engine offline. Still fail-closed where the READER itself fails: a MISSING/non-executable reader (the `! -x` guard), or any non-zero rc, parks the tick |
| fleet-halt.sh | the host-side R9 FLEET HALT reader (mirrors the dev-side `bin/fleet-halt.sh`, fedora-dev#168). One job: tell a sweeper whether the maintainer SOFT STOP is asserted. Reads the `halt` label on the FLEET HALT CONTROL issue in the control repo (`$FLEET_HALT_ORG/$FLEET_HALT_REPO`, discovered BY TITLE — live: #128) from the label's OWN TIMELINE EVENTS: the most-recent event whose actor holds admin\|maintain decides (labeled⇒HALTED, unlabeled⇒CLEAR), so it is MAINTAINER-BOUND BOTH DIRECTIONS — an App/bot label add OR removal is INERT (a definitive 404 on the role check ⇒ not a collaborator ⇒ inert; NOT unreadable). **AN UNREADABLE SIGNAL IS NOT A HALT** (fail direction INVERTED 2026-07-30, porting the dev side's fedora-dev#274 STEP 3; the law it now obeys is `fleet-core.md`'s *"ONLY a READ, PRESENT, maintainer-applied label halts"*): issue ABSENT ⇒ CLEAR, and an UNREADABLE read (discovery/timeline API error, or a role check GitHub could not answer) ⇒ **also CLEAR** — logged, and logged LOUDLY past K consecutive unreadable reads, but it NEVER escalates to a halt (the K counter now governs LOUDNESS only, not whether work proceeds). Emits `CLEAR`(0)/`HALTED`(10); `PAUSED`(11)/`HALTED-UNREADABLE`(12) are **RETIRED** — still documented as rc-contract history, never produced. Still fail-closed where the READER fails rather than the signal: callers treat every non-zero rc (a missing reader's 127 included) as observe-only. `--selftest` unit-tests the pure decision core; mock end-to-end in `validation/fleet-halt-dryrun.test.sh` |
| live-gate-run.sh | clone-on-demand + orchestration for one PR: fetches ONLY the PR head into an ephemeral tree, applies the STRUCTURAL GUARD (gateable only with a top-level `.live-gate`/`Containerfile*`, else neutral `SKIPPED`), PARSES the in-repo `.live-gate` (never `source`s it), builds + gates every declared target, posts the combined GREEN/RED verdict comment, tears the tree down. The host comments, never merges. **CAT-04 (audit 2026-07-18 — the likely kd#23 ROOT CAUSE):** a transient PR-head FETCH FAILURE now exits **2** (infra non-verdict → `live-gate-watch.sh` re-gates next poll), NOT 3 — because it posts NO comment, and rc 3 is DEDUPED as a "delivered SKIP", which buried the sha forever with nothing ever on the PR (the poller then reads host=NONE → NOOP indefinitely). rc 3 now means ONLY a genuinely-delivered structural skip, honouring the watcher's dedup-only-delivered contract. Covered by `validation/live-gate-fetchfail.test.sh`. **HEAD-SHA COHERENCE (#267, 2026-07-28 — the same bury CLASS, a different cause):** the watcher dedups under the sha `gh pr view --json headRefOid` reported, while this runner fetches `refs/pull/<n>/head` — a DERIVED ref GitHub updates ASYNCHRONOUSLY that can still carry the PREVIOUS head for a minute after a push. When they disagreed the gate posted a verdict for the OLD head and returned a VERDICT code, so the watcher buried the NEW one ("already gated", no verdict ever on the PR, dev consumers read host=NONE → poller NOOP). The caller now hands its dedup sha as optional arg 3 and the runner **REFUSES to gate any other sha** — exit **2** (infra non-verdict: nothing built, NO comment, NOT deduped → re-gate next poll, by which time the ref has caught up). Omitting arg 3 (a standalone/manual run) is unchanged. Covered by `validation/live-gate-shacoherence.test.sh`. The guard is **PROSPECTIVE ONLY** — it cannot un-write a marker the pre-fix code already wrote, so a head buried BEFORE it shipped stays buried (skipped silently forever; the R18 anomaly gate is the only thing that surfaces it). Recovery is manual: `rm ~/.local/state/live-gate/<repo>-<full-40-hex-sha>.done` on the host, or push a new commit (a new sha is a new dedup key). Deliberately not self-healing — auto-reaping a marker whose sha has no verdict comment would trust comment-ABSENCE (an API read that fails OPEN) to delete the only record that a sha was gated |
| validation/live-gate-fetchfail.test.sh | CAT-04 regression (audit 2026-07-18): drives the REAL `live-gate-run.sh` with a stub `git` whose `fetch` fails, asserts a PR-head fetch failure exits **2** (infra non-dedup → re-gate), reaches the fetch-fail path, and posts ZERO comment. **MUTATION RUN IN-SUITE** (the mutant sits BESIDE the real script so its `build-candidate.sh` preflight passes and it reaches the fetch): restore the old `exit 3` and the fetch-fail is DEDUPED (buried) — proving the non-dedup exit is what stops the kd#23-class bury. `bash validation/live-gate-fetchfail.test.sh` → exit 0. No GitHub/network. Run after touching the live-gate rc contract |
| validation/live-gate-shacoherence.test.sh | HEAD-SHA COHERENCE regression (#267, 2026-07-28): drives the REAL `live-gate-run.sh` with a stub `git` whose fetch "lands on" a caller-chosen sha, asserting a LAGGING `refs/pull/<n>/head` (fetched OLD head, caller dedups NEW) exits **2** with **ZERO comment posted**, while a coherent head and an arg-3-less standalone run gate normally (no false-positive, backwards compatible) and a malformed expected sha is refused. Also asserts the **WIRING** — `live-gate-watch.sh` passes `$sha` — since an unpassed sha makes the guard inert. **MUTATION RUN IN-SUITE** (the mutant sits BESIDE the real script so its `build-candidate.sh` preflight passes): neutralize the guard and the OLD head is gated + commented while the caller dedups the NEW one = the #267 bury, reproduced. `bash validation/live-gate-shacoherence.test.sh` → exit 0. No GitHub/network. Run after touching the live-gate dedup/sha contract |
...
| host-agent-watch.sh | **the HOST AUTONOMOUS AGENT** (apparatus fedora-dev#131 R5) — the symmetric half of the dev-side poller. Consumes dev→host **`host-task` GitHub-issue tickets** from the **control repo** (`$ORG/$REPO`, default `oso-gato/fedora-bootstrap` — host commands are scoped to the host's own repo, NOT org-wide, so authorization is the host repo's collaborators; `gh issue list` per tick — immediate, no search-lag, dep-free; flock singleton), parses the LINE-1 instruction `host-op: <verb> [args]`, dispatches to a fixed **verb ALLOWLIST** (**no arbitrary-command verb by design**; workload arg must be in the known set), reports the true outcome (DONE/DEFERRED/FAILED-rolled-back via `ExecMainStatus`) as a `host-agent:` comment + best-effort label + closes the ticket. **Verbs**: (1) `redeploy <workload>` → `workload-refresh@<workload>` (reversible; label-authorized). (2) `rebuild-devbox <devbox>` (**R17**, fedora-dev#174 host half) — the DESTRUCTIVE dev-box lifecycle rebuild the dev box cannot do to itself: KILL from OUTSIDE (host podman-level teardown reaps the shared-PID-ns ghost an in-container `distrobox rm -f` leaves) → REBUILD via `workload-rebuild@` (container-refresh under `FORCE_REBUILD`, R10 health-gate + rollback kept) → verify the kill **BY CONTAINER ID** → RESTORE+RESUME every **session-manifest** session (manifest = bounded, validated DATA in the ticket body between `%%DEVBOX-MANIFEST-BEGIN/END%%`, `parse_manifest`, never executed; each line `session <name> <cwd> [<sid>]` — the optional 4th field is a claude-code session-id (a valid UUID, D4/fedora-dev#191): PRESENT ⇒ resume THAT session by id `claude --resume <sid>` so MULTI-TENANT restore does not collapse N sessions sharing one cwd to the most-recent; ABSENT ⇒ the v1 cwd-scoped `claude --continue`; strict-UUID validation also re-closes the space-in-cwd hole). **RESUME-TO-ACTIVE (R17 option-b, 2026-07-17)** — a resumed session must come up ACTIVELY CONTINUING, not merely present (established empirically on fedora-dev with tmux + the real claude TUI: `claude --resume <sid>` restores context but comes up IDLE-at-the-input — a positional prompt only PRE-FILLS, never submits — and a live session's transcript does not flush in-tick, so it is not a usable liveness signal). So (a) a **BOX-READY gate** (`box_ready`: `.assembled` present, `.assemble-failed` absent, box enterable) is re-checked ACROSS ticks (DEFER-until-ready, deadline-bounded → FAILED if it never assembles) so the launch never lands in a still-assembling box and falls back to a bare shell — **the 0/N-idle race that resumed 0/2 on the first real rebuild**; then (b) each restored session is SUBMITTED a host-fixed continue-**NUDGE** (literal text + a DISCRETE `send-keys Enter` — an inline return is swallowed by the TUI's bracketed paste) asking it to `touch` a per-sid marker, and the executor polls that marker (shared home volume, base-visible) — a **filesystem HANDSHAKE** that CONFIRMS the session received+submitted+executed = actively continuing. Honest three-way tally: handshake-confirmed = ACTIVELY CONTINUING; a claude that is UP but unconfirmed, and a bare-shell IDLE, are NAMED distinctly and never claimed working → VERIFY the poller is observably sweeping → report killed/restored/actively-continuing/could-not (any unverified kill / unhealthy rebuild / box-never-ready / unconfirmed-or-idle session / silent poller SURFACES as FAILED — silence never means restored). Decoupled state machine (fire `--no-block` + poll across ticks via a `.rebuild` marker so a minutes-long rebuild never wedges the 300s tick); **AUTONOMOUS — no human tap** (**R41 DEPLOY-TO-LIVE AUTONOMY**, which SUPERSEDED the v1.2.69 R17 approval gate): it destroys running work, so it is authorized by a machine-proven RECOVERABILITY precondition rather than a maintainer act — the verb captures the session manifest FRESH from the LIVE box and REFUSES to fire when that capture fails (rc≠0 ⇒ `failed`, no kill), so it never destroys what it cannot restore. Authority (verifiable, and quoted in the script's DESTRUCTIVE-VERB AUTHORIZATION header): `oso-gato/fedora-dev` → `00-REQUIREMENTS.md` **R41** + `00-GOVERNANCE.md` **§6(g)** (the maintainer's decision 2026-07-27), both landed on fedora-dev `main` in **fedora-dev#261**, merged by the maintainer — the constitution lives in fedora-dev, NOT in this repo. The former gate's helpers (`is_authorized_author`/`approved_by_maintainer`/`approval_fold`) remain in-tree but **UNWIRED — they gate nothing**. **Decoupled idempotency**: `.acted` (written the instant the mutating op runs) guards the action so a delivery retry never re-acts; `.done` (written on delivery) guards re-delivery. (3) `apply-bootstrap` (**#133**) — HOST SELF-APPLY: make merged control-repo `main` LIVE on the host with no human (the last routine manual act: the maintainer's `git pull && setup.sh`). Fires the `host-apply.service` --user oneshot (→ `sudo /usr/local/sbin/host-apply`, ONE pinned NOPASSWD entry) which FF-pulls the control clone + re-runs the FULL setup.sh AS ROOT, health-gated with rollback + a fail-closed live READBACK (`applied` = `proven live`, #133 BLOCKING clause), REFUSING a dirty/diverged (non-FF) clone, IDEMPOTENT (same-sha ⇒ no-op). The SYSTEM-layer analogue of the F16 user-layer absorber. Long-running, so DECOUPLED like rebuild-devbox (`--no-block` + poll across ticks via a `.applyfired` marker + `ExecMainStatus` verdict map). LABEL-authorized (no arg; it applies pinned, merge-gated `main`, injecting nothing — the merge gate is the content-authorization). **Increment 2 — RECREATE ESCALATION:** on a SUCCESSFUL apply, it reads host-apply.sh's `quadlet-changed` signal and for each changed REBUILDABLE dev box FILES a `rebuild-devbox` recreate (`file_recreate_ticket`) — so a changed Quadlet `Environment=`/`Secret=` actually takes effect on the RUNNING container. It REUSES the proven `do_rebuild_devbox` path UNCHANGED, and since **R41** that bot-filed ticket is consumed AUTONOMOUSLY (kill → recreate → restore + resume every session), the session drop being gated by the fail-closed manifest capture rather than a maintainer's tap. IDEMPOTENT (an open recreate ticket for the same dev box + apply is never re-filed); an unchanged Quadlet files nothing. The load-bearing safety is the root-owned executor (`host-apply.sh`), agent-unmodifiable. Runs INSIDE claudebox (gh + CONTAINER_HOST bridge); `--selftest` exercises the pure parser + manifest validator + kill-verdict; the full lifecycle is mutation-checked in `validation/rebuild-devbox-dryrun.test.sh`. Driven by `host-agent-watch.timer` (10s), stood down during a box rebuild |
| systemd-units/ | instance templates for workload-refresh + retry, the on-demand `workload-rebuild@` (R17 force-rebuild executor), the two watcher units, + `claudebox-up.service` |
| systemd-units/workload-rebuild@.service | **R17 rebuild-devbox executor** — an on-demand (no timer) force-rebuild oneshot the host-agent starts with `--no-block`. Runs `container-refresh.sh %i` with `Environment=FORCE_REBUILD=1`, so it REUSES the health-gate + digest auto-rollback (not a duplicate) while overriding the busy-defer + digest short-circuit. `TimeoutStartSec=1800` (matches `workload-refresh@`); no `SuccessExitStatus=10` (FORCE never defers — a rolled-back rebuild exits 1 = unit failed = host-agent reports FAILED-rolled-back) |
| host-apply.sh | **the HOST SELF-APPLY EXECUTOR** (#133; apparatus F16b) — the ROOT-owned `/usr/local/sbin/host-apply` the `apply-bootstrap` host-agent verb triggers to make merged control-repo `main` live on the host with no human: the SYSTEM-layer analogue of the F16 user-layer absorber (which cannot run setup.sh — no host root). It fetches merged `main` from `origin` ONLY after matching the clone's `remote.origin.url` against an **anchored allowlist** of the canonical control-repo forms (a whole-string compare, NOT a `*substring*` — a repointed origin that merely CONTAINS the slug, e.g. `file:///…/oso-gato/fedora-bootstrap`, is REFUSED, so the fetch source is only ever the real control repo; the clone is core-writable, so this guard is load-bearing), refuses a **dirty/diverged (non-fast-forward)** clone (a question, not a force-pull), materialises the exact fetched sha into a **root-owned `git archive`** and runs THAT setup.sh AS ROOT (so the core-writable working tree injects nothing), HEALTH-GATES with verify.sh, ROLLS BACK to the prior commit + re-converges on failure/unhealthy, and records success only after a **FAIL-CLOSED live READBACK** byte-compares every installed artifact (user-layer via the reused `hcr_verify_from` + system-layer `/usr/local/sbin` scripts) to merged main (`applied` = `proven live`, #133 BLOCKING clause R17/#179). IDEMPOTENT (same-sha ⇒ no-op). **Increment 2 — QUADLET-CHANGE DETECTION:** a re-run of setup.sh may rewrite a deployed workload Quadlet (`~core/.config/containers/systemd/<name>.container`, e.g. #229 uncommenting the fitness `Secret=`/`Environment=`), which changes the file on disk but NOT the RUNNING container's env; so it sha256's each Quadlet BEFORE vs AFTER the apply (pure `ha_changed_quadlets`) and writes the CHANGED workload names to a WORLD-READABLE signal (`quadlet-changed`, the host agent reads it to file a recreate — consumed autonomously since R41, no maintainer tap; written on every rc-0 path, empty on a no-op, so a stale signal is never read). Exit contract 0/1/2/3 (applied / rolled-back / readback-mismatch / refused) → `ExecMainStatus` → the ticket verdict. Reached ONLY via `host-apply.service` → the pinned `CLAUDEBOX_APPLY` sudoers entry; `sudo` env_reset strips the `APPLY_*` test seams so the agent can inject nothing in production. `--selftest` covers the pure decision core; the full mock (clean/refuse/rollback/readback-mutation) is `validation/host-apply.test.sh`. **Control-plane (host self-apply).** Installed to `/usr/local/sbin` by `setup-host.sh` (root) — the one-time bootstrap manual apply, since the self-apply verb cannot install itself |
| systemd-units/host-apply.service | the `--user` oneshot the box-resident host agent fires (`systemctl --user start --no-block`) to run the self-apply. `ExecStart=sudo /usr/local/sbin/host-apply` — the ONLY escalation to root, through one pinned NOPASSWD entry. `ExecCondition` skips it during a claudebox rebuild (a mid-rebuild setup.sh would re-arm stood-down timers); `TimeoutStartSec=1800` (a full idempotent re-run is minutes, but it is fired `--no-block` + polled across ticks so it never wedges the 300s host-agent tick). In the F16 managed manifest (installed + daemon-reloaded by setup-user.sh + the absorber) |
| validation/host-apply.test.sh | MOCK end-to-end of `host-apply.sh` against a REAL git mock control clone (zero host/root/systemd/network): FF-apply + readback-pass, up-to-date no-op, dirty/diverged REFUSE, health-gate-fail → ROLLBACK, the **#133 readback MUTATION** — the setup stub injects a STALE `/usr/local/sbin/host-apply` and the REAL readback must FAIL closed (exit 2, no `applied.sha`); a paired guard mutation neutralises the readback and proves it is load-bearing — **and (increment 2) the QUADLET-CHANGE signal**: the setup stub rewrites a deployed `fedora-dev.container` and the executor's before/after sha diff must list `fedora-dev` in the signal (the recreate trigger), while an identical rewrite writes an EMPTY signal (no spurious recreate). `bash validation/host-apply.test.sh` → exit 0. Run after touching the host-apply seam |
...
| policy/sudoers.claudebox | scoped passwordless-sudo allowlist for the operating user; visudo-validated, stamped to /etc/sudoers.d/claudebox. Two aliases: `CLAUDEBOX_HOST` (pinned `tailscale serve`/`status`, no wildcards) + `CLAUDEBOX_APPLY` (#133: exactly `/usr/local/sbin/host-apply`, argument-free — the ONE scoped root-apply grant; the executor owns its own inputs + applies only pinned merge-gated `main`, so the sudoers surface exposes no injectable argument) |
| verify.sh | PASS/FAIL acceptance: sockets, box, claude, policy, host-engine reach, tailnet, box-rebuild units, workload-refresh timers, dnf-automatic timer, fail2ban absent (key-only door), firewalld absent (leaf footprint), sudo doctrine boundary, SELinux config not disabled (permissive or enforcing), **host self-refresh armed** (F16 absorber timer enabled + control clone core-writable + last heartbeat not BLOCKED/FAILED — a non-self-refreshing host is a LOUD FAIL, not silence), **host self-apply wired** (#133: `/usr/local/sbin/host-apply` executor installed + `host-apply.service` present + the agent holds the pinned `host-apply` sudo grant — a silently-unwired self-apply is a LOUD FAIL) |
| host-code-refresh.sh | **the F16 SELF-ARMING ABSORBER** (R23 host half) — the "merged host code → live" mechanism the host lacks otherwise (no product image/CI artifact: host control code goes live only when `main` is pulled into the control clone + the user install re-runs). On a ~15-min `--user` timer it FAST-FORWARDS the control clone to merged `main` and re-runs the idempotent install (`hcr_install_from`, the SINGLE manifest source `setup-user.sh` also uses) with a **FAIL-CLOSED live READ-BACK** (`hcr_verify_from`: each installed artifact byte-compared to the merged source; a mismatch records NO `applied.sha`, exits non-zero). FAIL-CLOSED + never-clobber: a DIRTY/DIVERGED clone is REFUSED untouched (strict `--ff-only`, never `reset --hard`); every failure degrades to the status quo and never stops a future tick. **Requires the control clone `core`-writable** (setup-host.sh chowns it) — else a fail-closed no-op. **HEARTBEAT (2026-07-17):** every tick records `<epoch> <outcome>` to `$state/status` (`OK`/`BLOCKED`/`SKIPPED`/`FAILED` + reason) so a blocked/silent absorber is VISIBLE off-box (verify.sh reads it) — the fix for the incident where the host silently sat on hand-applied code for days. `--selftest` covers the pure decision core + manifest shape; `validation/host-code-refresh.test.sh` drives it end-to-end against a real git mock clone (advanced/dirty/diverged/up-to-date/readback-mismatch/not-writable→BLOCKED + a heartbeat mutation). **Control-plane (host self-refresh).** |
