...
| systemd-units/claudebox-up.service | **the claudebox OWNER** (incident 2026-07-11). Starts the box with `env -u INVOCATION_ID podman start claudebox` so podman creates the container's OWN `libpod-conmon` scope (it skips scope creation when `INVOCATION_ID` is set — the exact bug: conmon then lands in the *starting* unit's cgroup and dies with it, exit 143). oneshot (NOT RemainAfterExit — stays re-runnable so each watcher's `Wants=` re-fires it and revives a dead box); rebuild-guarded via ExecCondition; `podman start` is a no-op on a running box. Enabled + started by setup-user.sh BEFORE the watcher timers; stood down + re-armed by box-rebuild.sh. This is what keeps a watcher-tick cgroup teardown from killing the box |
