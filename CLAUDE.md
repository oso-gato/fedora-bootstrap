...
| validation/host-apply.test.sh | MOCK end-to-end of `host-apply.sh` against a REAL git mock control clone (zero host/root/systemd/network): FF-apply + readback-pass, up-to-date no-op, dirty/diverged REFUSE, health-gate-fail → ROLLBACK, and the **#133 readback MUTATION** — the setup stub injects a STALE `/usr/local/sbin/host-apply` and the REAL readback must FAIL closed (exit 2, no `applied.sha`); a paired guard mutation neutralises the readback and proves it is load-bearing (the stale case then wrongly passes). `bash validation/host-apply.test.sh` → exit 0. Run after touching the host-apply seam |
