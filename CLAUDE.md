...
| 10 | SIGNATURE ENFORCEMENT (fleet baseline — host side) | The host **enforces** the fleet signature flow: `~/.config/containers/policy.json` verifies every workload image as **`sigstoreSigned { keyData: <fleet public key> }`** — podman-native, **no cosign on the host**. **Key-based, NEVER keyless** (podman `policy.json` matches only a `subjectEmail`, never a GitHub-Actions workflow-URI identity). This is the host half of the **ground-rule baseline every image repo builds in** — what lets the host safely run *un-merged* candidate images in the autonomous pre-merge dev loop (verify CI provenance before running). Flip a workload's stanza from the scaffolded `insecureAcceptAnything` to `sigstoreSigned` once that workload's CI key-signs (so the pull never breaks); the flip is an operator-applied control-plane step. |
