...
| setup-user.sh | **rootless layer**, as `core` — user podman socket, ssh keys, claudebox, Claude policy, the `claude` + `claudebox-rebuild` wrappers + the box-rebuild units, workload-refresh harness, verify (no host privilege). Installs (`hcr_install_from`) + enables + **PRIMES** the F16 absorber once so the host is current the moment setup finishes and verify has a fresh heartbeat. **Persists the dev-box poller's autonomous-merge arming** into the installed `fedora-dev` Quadlet ((b5), incident #198) so a container recreate keeps the poller+deadman alive instead of coming up dead+silent — a deploy-spec line, not a live injection; fail-closed (only `fedora-dev`, only with its standing App) |
