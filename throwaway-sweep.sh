...
#   FD_DNF_CACHE_CAP_GB    cap the dnf cache; prune oldest RPMs over   (default 5)
...
DNF_CAP_GB="${FD_DNF_CACHE_CAP_GB:-5}"
...
echo "[sweep] start: age>${AGE_MIN}m orphans + cache GC (dnf cap ${DNF_CAP_GB}G, build-cache until=${BUILDCACHE_AGE})${DRY:+ [DRYRUN]}"
...
# ---- (4) BOUNDED dnf RPM bind-cache: keep newest RPMs up to the cap, prune the oldest beyond it ----
if [ -d "$DNF_CACHE" ]; then
  ...
  if [ $(( cur_kb * 1024 )) -gt "$cap_bytes" ]; then
    echo "[sweep] dnf cache $(( cur_kb / 1024 ))M > cap ${DNF_CAP_GB}G — pruning oldest RPMs"
    running=0
    ...
    while IFS=$'\t' read -r _t sz path; do
      ...
      if [ "$running" -gt "$cap_bytes" ]; then
        run rm -f "$path" && echo "[sweep] dnf-cache GC removed $(basename "$path")"
      fi
