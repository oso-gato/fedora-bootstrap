...
ck "host: no shim symlinks in ~/.local/bin" "! ls ~/.local/bin/podman ~/.local/bin/systemctl"
ck "box: exists"                           "distrobox list | grep -q claudebox"
ck "box: claude runs"                      "distrobox enter claudebox -- claude --version"
ck "box: policy present"                   "distrobox enter claudebox -- test -f /etc/claude-code/CLAUDE.md"
